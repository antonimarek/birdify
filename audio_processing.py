...
class Visualize:
    def __init__(self, signal: dict = None, fft: dict = None):
        self.signal = signal
        self.fft = fft
    ...
    def fft_plt(self, plot_size: tuple = (20, 5), n_col: int = 2):
        key_list = list(self.fft.keys())
        x_arg = [self.fft[key][1] for key in key_list]
        y_arg = [self.fft[key][0] for key in key_list]
        self.plot_prepare(plot_size, key_list, x_arg, y_arg, n_col=n_col)
        plt.show()
        pass
